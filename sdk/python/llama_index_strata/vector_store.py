"""Strata vector store for LlamaIndex.

Implements the LlamaIndex ``VectorStore`` interface so Strata can serve
as a drop-in vector backend for any LlamaIndex pipeline.

Usage::

    from llama_index.core import VectorStoreIndex, StorageContext
    from llama_index_strata import StrataVectorStore

    vector_store = StrataVectorStore(url="http://localhost:8432")
...
class StrataVectorStore:
    """LlamaIndex-compatible vector store backed by the Strata context lake.

    This store delegates embedding and storage to Strata's server-side pipeline.
    Documents are ingested via ``/api/v1/ingest`` (auto-embedded by Strata's
    ...
    def add(
        ...
        **kwargs: Any,
        ...
        Strata's ingest pipeline handles embedding automatically.
        """
    ...
    def delete(self, ref_doc_id: str, **kwargs: Any) -> None:
        ...
        Note: Strata's episodic store is append-only by design.
    ...
    def query(
        ...
        **kwargs: Any,
        ...
        or a plain string. Returns results from Strata's embed-and-search endpoint.
        """
    ...
    def _to_query_result(self, results: list[dict[str, Any]]) -> Any:
        """Convert Strata results to VectorStoreQueryResult if possible."""
        if _HAS_LLAMA_INDEX:
            nodes: list[TextNode] = []
