"""CrewAI tool wrappers around the Strata Python SDK.

These tools let CrewAI agents interact with Strata's memory stores:
- StrataSearchTool: Semantic search over episodic events
- StrataQueryTool: SQL queries against the episodic store
- StrataStateTool: Agent state get/set
- StrataIngestTool: Ingest new events
"""
...
class StrataSearchTool(BaseTool):
    """Search Strata's memory using natural language."""

    name: str = "strata_search"
    description: str = (
        "Search Strata's episodic memory using natural language. "
    ...
    def _run(self, query: str) -> str:
        async def _search() -> list[dict[str, Any]]:
            async with StrataClient(self.url) as client:
...
class StrataQueryTool(BaseTool):
    """Execute SQL queries against Strata's episodic store."""

    name: str = "strata_query"
    description: str = (
        "Execute a SQL query against Strata's episodic event store. "
...
class StrataStateTool(BaseTool):
    """Get or set agent state in Strata."""

    name: str = "strata_state"
    description: str = (
        "Get or set agent state in Strata's key-value store. "
...
class StrataIngestTool(BaseTool):
    """Ingest events into Strata's episodic memory."""

    name: str = "strata_ingest"
    description: str = (
        "Ingest a new event into Strata's episodic memory. "
