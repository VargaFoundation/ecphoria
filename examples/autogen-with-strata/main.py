"""AutoGen + Strata: Using Strata as a persistent memory backend for AI agents.

This example demonstrates how AutoGen agents can use Strata for:
...
async def seed_strata() -> None:
    """Ingest sample knowledge into Strata."""
    async with StrataClient(STRATA_URL) as client:
        count = await client.ingest("knowledge-base", SAMPLE_KNOWLEDGE)
        print(f"Seeded {count} knowledge entries into Strata")


# ── Strata-backed functions for AutoGen ──────────────────────────
...
def recall_context(
    ...
    """Search Strata's memory for relevant past context using semantic search."""
...
def save_memory(
    ...
    """Save an observation or finding to Strata's episodic memory for future recall."""
...
def get_agent_state(
    ...
    """Retrieve persistent agent state from Strata."""
    async def _get() -> Any:
        async with StrataClient(STRATA_URL) as client:
...
def set_agent_state(
    ...
    """Persist agent state in Strata for future conversations."""
    async def _set() -> int:
...
def build_agents() -> tuple[autogen.AssistantAgent, autogen.UserProxyAgent]:
    ...
    assistant = autogen.AssistantAgent(
        name="strata_assistant",
        system_message=(
            "You are a helpful assistant with access to a persistent memory system (Strata). "
    ...
    # Register Strata functions with both agents
