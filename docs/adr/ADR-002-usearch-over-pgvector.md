...
Strata's semantic memory stores vector embeddings alongside event metadata, enabling agents to search for relevant past experiences by meaning rather than keywords. We need a vector search engine that:
...
**pgvector**: The default choice for Postgres-based stacks. Excellent if you're already running Postgres, but Strata doesn't use Postgres — we use DuckDB for episodic and SQLite for state. Adding a full Postgres dependency solely for vector search contradicts the single-binary goal.

**Qdrant / Milvus / Weaviate**: Purpose-built vector databases with rich feature sets (filtering, multi-tenancy, replication). But they all run as separate services with their own ports, configs, and upgrade cycles. They solve a harder problem than we need — Strata's semantic store is one component within a larger system, not a standalone service.
...
- **Post-filtering**: USearch doesn't support native metadata filtering. We over-fetch by 2x and filter in application code. For Strata's scale (thousands to low millions of vectors per node), this is efficient enough.
...
- **No native filtering**: Metadata filtering must happen in application code (post-filter). For high-cardinality filters with large indexes, this means over-fetching. At Strata's target scale this is acceptable, but it won't match the efficiency of Qdrant's native filtered search.
