//! `strata doctor` — a static configuration linter.
//!
//! Loads a `strata.toml` (plus the `STRATA_*` env overrides for the fields it checks) and reports
...
pub async fn run(config_path: Option<&str>) -> anyhow::Result<()> {
    ...
    let toml_val: toml::Value = match std::fs::read_to_string(&path) {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            ...
            eprintln!("note: {path} not found — linting defaults + STRATA_* env only\n");
...
/// `STRATA_GATEWAY__AUTH_ENABLED` env var taking precedence (mirrors the server's layering).
fn cfg_str(toml_val: &toml::Value, env: &EnvLookup, dotted: &str) -> Option<String> {
    let env_key = format!("STRATA_{}", dotted.to_uppercase().replace('.', "__"));
...
fn has_api_keys(toml_val: &toml::Value, env: &EnvLookup) -> bool {
    if let Some(s) = env.get("STRATA_GATEWAY__API_KEYS") {
...
fn lint(toml_val: &toml::Value, env: &EnvLookup) -> Vec<Finding> {
    ...
    if cfg_bool(toml_val, env, "cluster.enabled", false) {
        let secret = cfg_str(toml_val, env, "cluster.secret")
            .or_else(|| env.get("STRATA_CLUSTER__SECRET"))
            .or_else(|| env.get("STRATA_CLUSTER__SECRET_FILE"));
        if secret.is_none() {
            out.push(Finding {
                level: Level::Warn,
                message: "cluster.enabled=true but no cluster secret is set — Raft RPCs are \
                          unauthenticated. Set STRATA_CLUSTER__SECRET on every node."
                    .into(),
            });
        }
    }
...
fn print_findings(findings: &[Finding]) {
    if findings.is_empty() {
        println!("✓ strata doctor: no configuration issues found.");
        return;
    }
    ...
    print!("{buf}");
    println!("\nstrata doctor: {errs} error(s), {warns} warning(s), {infos} info.");
}
    ...
    fn env_overrides_toml() {
        ...
        let f = lint(
            ...
            &env(&[
                ...
                ("STRATA_GATEWAY__API_KEYS", "sk_live@acme:writer"),
            ]),
        );
        assert!(
            f.iter().all(|x| x.level != Level::Error),
