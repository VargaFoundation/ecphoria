...
//!
//! The agent driver (in `strata-core`) must be able to invoke **external** tools (e.g. downstream
//! MCP servers) without `strata-core` depending on the protocol layer. So core defines this trait
//! and the gateway's MCP tool-gateway implements it; the gateway injects an implementation into the
//! engine via [`crate::StrataEngine::set_tool_executor`].
