...
//!
//! Strata extends DuckDB SQL with virtual functions:
//! - `strata_search('text', k)` — semantic similarity search
//! - `strata_state('agent_id', 'key')` — state key lookup
//!
...
/// Rewrite a SQL query containing strata_search() or strata_state() calls
...
/// FROM episodic e
/// JOIN strata_search('billing issue', 3) s ON true
/// ```
///
/// Becomes:
/// ```sql
/// WITH _strata_search AS (
///   SELECT '...' AS id, '...' AS content, '{}' AS metadata, 0.95 AS score
///   UNION ALL SELECT ...
/// )
/// SELECT e.source, s.content, s.score
/// FROM episodic e
/// JOIN _strata_search s ON true
/// ```
pub async fn rewrite_hybrid_query(
    ...
    // Check if this query contains strata functions that need rewriting
    let has_search = upper.contains("STRATA_SEARCH(") || upper.contains("STRATA_SEARCH (");
    let has_state = upper.contains("STRATA_STATE(") || upper.contains("STRATA_STATE (");
    ...
    // Handle strata_search() calls
    if has_search {
        if let Some(args) = extract_function_args(sql, "strata_search") {
            ...
            let results = if let Some(provider) = embedding {
                ...
                return Err(crate::Error::Embedding(
                    "no embedding provider configured for strata_search()".into(),
                ));
            };
            ...
            ctes.push(format!("_strata_search AS ({})", cte_body));

            // Replace the function call in the SQL with the CTE reference
            // Match patterns like: strata_search('text', k) or strata_search('text')
            let func_pattern = find_function_call(sql, "strata_search");
            if let Some((start, end)) = func_pattern {
                rewritten = format!("{}_strata_search{}", &sql[..start], &sql[end..]);
            }
        }
    }

    // Handle strata_state() calls
    if has_state {
        if let Some(args) = extract_function_args(&rewritten, "strata_state") {
            let parts: Vec<&str> = args.splitn(2, ',').collect();
            let agent_id = parts
                .first()
                .unwrap_or(&"")
            ...
            ctes.push(format!("_strata_state AS ({})", cte_body));

            let func_pattern = find_function_call(&rewritten, "strata_state");
            if let Some((start, end)) = func_pattern {
                rewritten = format!("{}_strata_state{}", &rewritten[..start], &rewritten[end..]);
            }
        }
    }
...
mod tests {
    ...
    fn find_function_call_basic() {
        let sql = "SELECT * FROM strata_search('hello', 3)";
        let (start, end) = find_function_call(sql, "strata_search").unwrap();
        assert_eq!(&sql[start..end], "strata_search('hello', 3)");
    }

    #[test]
    fn find_function_call_in_join() {
        let sql = "SELECT e.* FROM episodic e JOIN strata_search('test', 5) s ON true";
        let (start, end) = find_function_call(sql, "strata_search").unwrap();
        assert_eq!(&sql[start..end], "strata_search('test', 5)");
    }

    #[test]
    fn find_function_call_missing() {
        let sql = "SELECT * FROM episodic";
        assert!(find_function_call(sql, "strata_search").is_none());
    }

    #[test]
    fn extract_args_basic() {
        let args = extract_function_args("strata_search('hello', 3)", "strata_search");
    ...
    fn extract_args_in_context() {
        let args = extract_function_args(
            "SELECT * FROM episodic JOIN strata_state('bot', 'mood') ON true",
            "strata_state",
        );
