...
| MCP Server | **Working** | **Streamable HTTP** at /mcp: POST (JSON-RPC; initialize returns `Mcp-Session-Id`) + GET (server→client SSE keep-alive) → native Claude Code **and** Claude Desktop. initialize, tools/list (25 tools), tools/call, resources/list, prompts/list. Authenticated when `auth_enabled`. See docs/connect-claude.md |
| MCP tools | **Working** | 25 tools: query (incl. `SELECT … FROM memories`), ingest, get_state, set_state, search, embed, start/end/recall_session + **memory** (add_memory, search_memory, get_memories [filters+offset], memory_history, delete_memory, update_memory, memory_scopes, remember) + **graph** (link_memory, graph_neighbors) + **cognition/analytics** (memory_provenance, memory_feedback, list_contradictions, graph_centrality, graph_path, graph_communities) |
| gRPC | **Working** | tonic Query/Ingest/Search/State/Health **+ memory (add/search/get [filters+offset]/update/delete/scopes/provenance) + sessions (start/end/recall) + graph (centrality/path/communities, tenant-scoped, optional as-of)** RPCs, 16MB max message. **Typed payloads** (`google.protobuf.Struct`/`Value` for rows/events/metadata/state values — not JSON-in-string). **Authenticated** (Bearer JWT in metadata) + **tenant-scoped** when auth_enabled |
