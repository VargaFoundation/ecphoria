...
| MCP Server | **Working** | **Streamable HTTP** at /mcp: POST (JSON-RPC; initialize returns `Mcp-Session-Id`) + GET (server→client SSE keep-alive) → native Claude Code **and** Claude Desktop. initialize, tools/list (23 tools), tools/call, resources/list, prompts/list. Authenticated when `auth_enabled`. See docs/connect-claude.md |
| MCP tools | **Working** | 23 tools: query (incl. `SELECT … FROM memories`), ingest, get_state, set_state, search, embed, start/end/recall_session + **memory** (add_memory, search_memory, get_memories, memory_history, delete_memory, remember) + **graph** (link_memory, graph_neighbors) + **cognition/analytics** (memory_provenance, memory_feedback, list_contradictions, graph_centrality, graph_path, graph_communities) |
