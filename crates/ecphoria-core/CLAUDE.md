...
| `MemoryStore` (cognition) | **Working** | Bi-temporal `memories` (DuckDB): `valid_from`/`valid_to`, deterministic contradiction resolution (supersede), dedup/consolidation, importance, decay-based forgetting, `as_of`/history, scoped by tenant/user/agent/session. Hybrid retrieval = BM25 fused with vector via RRF. Separate USearch index (rebuildable). Mem0-compatible engine API |
| `facts` + `schema` | **Working** | `memory/facts.rs` — the `FactKind` vocabulary, the per-kind subject grammar, and validation of `metadata` against `schemas/facts/*.schema.json`. `memory/schema.rs` is a small JSON Schema 2020-12 **subset** validator: it refuses to compile a schema using a keyword it does not implement, so a `oneOf` added to a fact schema breaks the build instead of silently validating less. Enforcement is per tenant (`memory.governance`), off by default |
| `promote` (ingest) | **Working** | `ingest/promote.rs` — turns durable webhook outcomes (merged PR, closed issue, resolved incident) into memories with deterministic subjects, so redelivery confirms instead of duplicating. Rule syntax `"<vendor>:<event_type>"` with `*`; gated on `memory.promotion.enabled` |
| `chunk` (ingest) | **Working** | Structure-aware Markdown chunking (`ingest/chunk.rs`): splits on heading hierarchy, never inside a fenced code block, heading trail prefixed onto each chunk for retrieval context, oversized sections packed at paragraph boundaries. `Chunk::subject(path, part)` is the stable address that makes per-section bi-temporal history work. Deliberately no size-based merging (it would destabilise addresses). Driven by `EcphoriaEngine::document_ingest` |
| `LexicalIndex` | **Working** | SQLite FTS5 (`memory/lexical.rs`) — inverted index over `subject`+`content`, partitioned by exact scope, `porter unicode61 tokenchars '-_.'` (identifiers survive: `ECPHORIA_STORAGE__DATA_DIR`, `PROJ-1234`, `ecphoria-core`). Stage one of the two-stage lexical arm; `cognition::lexical_rank` is stage two. Advisory index — never the source of truth, so stale entries cost a candidate slot, never correctness. `engine.lexical_reindex()` rebuilds from DuckDB |
